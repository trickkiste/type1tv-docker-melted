...
RUN mkdir -p /tmp/blackmagic ; tar xvfz /tmp/Blackmagic_Desktop_Video_Linux_9.8.tar.gz -C /tmp/blackmagic ; dpkg -i /tmp/blackmagic/desktopvideo-9.8-amd64.deb ; rm -r /tmp/blackmagic 
...
RUN cd /tmp/ ; git clone https://github.com/trickkiste/mlt-scripts.git ; /tmp/mlt-scripts/build/build-melted.sh -c /tmp/build-melted.conf ; rm -r /tmp/melted ; rm /tmp/build-melted.conf ; rm -r /tmp/mlt-scripts
RUN apt-get install -y libmp3lame0 libgavl1 libsox1b libexif12 libvdpau1 libqt4-gui ; apt-get clean
# if you need Rugen
